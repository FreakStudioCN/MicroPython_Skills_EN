...
- An entry point for app store publishing.
...
- Seeds bundled apps to `/apps` on first run.
...
After reset, bundled demo apps will be re-seeded on first run.
...
The Web port is self-contained within the main MicroPythonOS repository. Submodule modifications required by the web target are stored in `scripts/web_port/` and applied automatically during the build. Unless a task explicitly requires modifying these projects, do not leave persistent changes in nested `lvgl_micropython`, `micropython`, or `lvgl` directories.
...
- For desktop work, prefer `make build-mpos-unix`; only use the web build when targeting browser/WebAssembly.
