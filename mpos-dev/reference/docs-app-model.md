...
This file is generated based on a re-read of the `docs.micropythonos.com` sitemap/search index from 2026-07-14, combined with corrections from `/home/leeqingshui/MicroPythonOS/AGENTS.md` and the current local repository facts.
...
The current local repository and `tests/test_apps_manifest.py` use a flat structure as the default:
...
`mpos-gen-app` generates a root-level `MANIFEST.JSON` and root-level `icon_64x64.png` by default. The entrypoint defaults to `assets/main.py`, so that runtime files placed in `assets/` by `mpos-prepare-deps` can be imported directly.
...
- `onResume(screen)`: Enters foreground, becomes interactive.
...
A Service is a background component without a UI. Suitable for startup tasks and long-running tasks, such as automatic WiFi connection, web server startup, async REPL tasks, periodic checks, notifications, etc.
...
Built-in Apps reside on the device under `/builtin/apps/`, including launcher, WiFi, AppStore, OSUpdate, Settings, File Manager. User-installed Apps reside under `/apps/`.
...
Most Apps should be written in MicroPython. Use C/C++ native modules only when pure MicroPython is insufficient, e.g., for high-frequency game loops, signal processing, calling C libraries, etc.
