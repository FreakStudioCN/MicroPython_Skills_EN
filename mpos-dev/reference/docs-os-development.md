...
Targets mentioned in the docs include `esp32`, `esp32s3`, `unix`, `macOS`, `web`. The local AGENTS also mention additional targets such as `esp32-small`, `unphone`, `lilygo_t4`.
...
2. Confirm the MicroPython REPL can start.
...
## When to Escalate from App Work to OS Work

Stay at the App-level skill unless the task requires:
