...
This file is generated based on the re-read of `https://docs.micropythonos.com/sitemap.xml` and `search/search_index.json` from 2026-07-14.

Purpose: Audit docs coverage. This round read all 61 pages sequentially according to the sitemap, with the result `DOCS_FETCH_OK=61/61 failed=0`; the docs search index contains 977 page/section entries. Specific work rules are split across the topic reference files below.
...
- `docs-frameworks.md`: Framework architecture and manager/service API.
...
- This is not a verbatim mirror of the docs site, but a coverage table and routing table for the split reference files.
- Each topic reference will paraphrase the docs content and add local `AGENTS.md` constraints where the public documentation is inconsistent with the local repository.
