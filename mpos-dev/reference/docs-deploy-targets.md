...
When cleaning up residual simulator processes, use `killall`, not `pkill -f`:
...
- When equivalent entry points exist, prefer using `make build-mpos-unix`, `make syntax-tests`, `make unittest-tests`, `make tests`, `make lint`, `make lint-fix`.
