...
`upy-flash-mpy-firmware-plugin` is the plugin phase after `select-hw`. It only consumes `phase_complete.select_hw.json`, does not re-analyze requirements, re-select boards, or generate business code.
...
| `runtime_context.session_root` | Yes | Relative session directory, typically `sessions/<session_id>`. |
| `runtime_context.resource_root` | Yes | Root directory for installed skills/resources; used to locate this skill's scripts. |
| `capabilities.script_run` | Yes | Host can run whitelisted skill scripts. |
| `capabilities.network_access` | Yes | Host can access MicroPython download pages or trusted vendor firmware sources. |
| `capabilities.serial_port_scan` | Required for real ESP32 flashing | Host can enumerate serial ports. |
| `capabilities.device_flash` | Required for real ESP32 flashing | Host allows erase/write after user confirmation. |
...
In V0, user confirmation of `copied_uf2` is sufficient for success.
...
| `firmware_download.py` | `--resolved-json`, `--out-dir` | `--output-json`; also compatible with `--out-json`; it is recommended to pass `--artifact-root` to output a relative `downloaded_artifact_path`. The ZIP branch also outputs `archive_artifact_path`. |
...
```json
{
  "phase": "upy-flash-mpy-firmware-plugin",
  "result": "success",
  "summary": "MicroPython firmware flashing phase complete",
