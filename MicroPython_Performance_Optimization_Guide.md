...
We can compare the MicroPython code optimization process to "steps to speed up a bicycle": first find the core reasons why the bike is slow (e.g., flat tires, chain friction), then start with simple adjustments (inflating tires, lubricating the chain), and finally consider replacing high-end parts (lightweight frame, carbon fiber wheels). MicroPython code optimization also follows a **"simple to complex, software to hardware"** order, allowing you to achieve the greatest performance improvement at the lowest cost, avoiding wasting time on complex low-level optimizations from the start.

Before starting optimization, understand a few core basic concepts:
...
1. **Identify the slowest code**: Use timing tools to find the real performance bottleneck, rather than optimizing based on intuition.
2. **Memory and object optimization**: Reduce heap allocation, avoid dynamic object creation, and lower the frequency of GC triggering.
3. **Code execution efficiency optimization**: Use `const()`, precompiled bytecode, native/viper code emitters.
4. **Computation and hardware optimization**: Replace floats with integers, directly manipulate registers, use DMA.

# 2. Optimization Methods

## 2.1 Identify the Slowest Code

To identify the slowest code, we can usually use the `utime` module's tick recording functions by strategically using timing. Code execution time can be measured in ms (milliseconds), us (microseconds), or CPU cycles.
...
Here, we use the mpremote tool to connect to a Raspberry Pi Pico, copy the code above into the REPL, and press Enter to execute (the decorator is now defined):
...
The function execution time is output:
...
## 2.2 Specific Performance Optimization Measures

### 2.2.1 MicroPython Performance Bottlenecks and Core Basic Concepts

#### 2.2.1.1 Core Basic Concepts
...
| Use case | Values that need to change during runtime | Fixed values like register addresses, max counts, pin masks |

##### 2.2.1.1.2 Memory Three-Partition
...
| **Heap** | Warehouse (large space, needs registration to retrieve) | All Python objects (lists, dicts, bytearrays, etc.) | Dynamic allocation, GC reclamation | Slow allocation, GC triggering is time-consuming |
...
- **Heap memory fragmentation**: After frequent allocation/release of small objects, free memory becomes non-contiguous, making it impossible to allocate large blocks of memory.
...
- **Stack overflow**: When recursion is too deep or there are too many local variables, the stack space is exhausted, causing a hardware exception.

##### 2.2.1.1.3 References and Copies

A reference is like the object's **"address number"** (analogy: a shortcut on a computer). The variable stores not the data itself, but the address of the data in memory. Operating on a reference does not copy the data, and the overhead is minimal.
...
For copying (here referring to deep copy; the difference between shallow and deep copy is not elaborated here), it copies the entire data, creates a new object on the heap, stores a new data set, and has significant overhead (especially for large data):
...
To avoid the large heap allocation problem when copying large bytearrays, we can use `memoryview`. It is a shallow reference tool provided by MicroPython, essentially a "read-only/writable address number" for buffer objects (bytearrays, arrays, bytes, etc.). Slicing with `memoryview` does not copy data, only passes the address, completely avoiding heap allocation.
...
> **Note**: `memoryview` only supports objects that implement the buffer protocol (bytearrays, arrays, bytes). It does not support lists (lists store object references, not contiguous data).

After mastering the basics above, understand these advanced concepts for embedded MicroPython:
...
| **Bytecode Interpretation** | MicroPython compiles `.py` to bytecode and interprets it instruction by instruction | 10 to 100 times slower than native C code |
...
| Optimization Level | Main Methods | Expected Benefit |
|-------------------|--------------|------------------|
| **Memory and Object Optimization** | Pre-allocation, fixed size, memoryview, cache references, manual GC | Eliminates random GC pauses, significantly improves stability |
| **Code Execution Efficiency Optimization** | `const()`, mpy-cross precompilation, native/viper decorators | Tens of times faster integer operations, faster loading |
| **Computation and Hardware Optimization** | Replace floats with integers, direct register access (mem32), DMA | Minimizes hardware operation latency, maximizes CPU resource utilization |

### 2.2.2 Memory and Object Optimization

The core of this type of optimization is to "avoid dynamically creating objects at runtime as much as possible, reduce heap allocation, thereby lowering the frequency and duration of GC triggering." This is the primary step in embedded MicroPython performance optimization.

#### 2.2.2.1 Pre-allocate Memory and Fix Object Size

Create objects only once (e.g., instantiate in the class constructor), and do not allow their size to grow dynamically (e.g., list `append`, adding key-value pairs to a dict). Especially for buffers (e.g., serial communication buffers), pre-allocate and reuse them.
...
Taking a serial buffer as an example:
...
Lists store object references, memory is non-contiguous, and dynamic growth triggers heap allocation; the `array` module (or `bytearray`) stores contiguous basic type data, offering higher performance after pre-allocation. Also, slice operations (e.g., `ba[30:2000]`) create data copies, triggering heap allocation; using `memoryview` can directly pass a memory pointer with no copy overhead.
...
> Caching references is more effective in scenarios with large loop counts (e.g., 10000+) and nested attributes (e.g., `self.obj.buf`).

#### 2.2.2.4 Manual Garbage Collection Control

Periodically call `gc.collect()` to manually trigger GC, preventing GC from being triggered randomly during performance-critical code sections (manual GC allows controlling the timing, and the time spent on frequent small GCs is much less than a single large GC).
...
### 2.2.3 Code Execution Efficiency Optimization

This type of optimization builds on memory optimization to further improve code execution speed, optimizing from the bytecode level to the machine code level.

#### 2.2.3.1 Use const() to Declare Constants

`const()` replaces an identifier with its value (done at compile time), avoiding runtime dictionary lookups. The optimization effect is significant, especially for constants used in loops.
...
> **Note**: The run times are nearly identical because in the REPL, code is interpreted, and the real advantage of `const` is evident in **precompiled bytecode**. When the code is saved as a `.py` file and executed via `import`, the optimization effect of `const()` is more significant.
...
`.mpy` files are smaller than `.py` files (microMLP.py 32KB → microMLP.mpy 10KB), skip the compilation stage at startup, and load faster.
...
| **viper Emitter** | Generates optimized machine code, focuses on integer/pointer operations | Tens of times faster for integer operations | Not fully compatible, requires type annotations |
| **Inline Assembler Emitter** | Directly embeds assembly instructions, extreme performance | Fastest | Requires assembly knowledge, poor portability |

##### 2.2.3.3.1 native Code Emitter

The native code emitter takes each bytecode and converts it into equivalent ARM-Thumb machine code. Such functions use the normal C stack to store local variables and directly call C runtime functions.
...
| `ptr32` | Pointer to 32-bit integer | Directly accessing register addresses |
...
# Viper optimized function: same computation
...
> **Result Analysis**:
> - `normal_add_loop` took **17221.732ms** (approx. 17.2 seconds)
> - `viper_add_loop` took **296.095ms** (approx. 0.3 seconds)
> - The Viper version is **about 58 times faster**!
> - Note: The return value of `viper_add_loop` is `-723379968`. This is because Viper uses 32-bit machine word integers, and 1 million accumulations exceed the 32-bit integer range (2^31 ≈ 2.1 billion), causing overflow truncation.

**Viper has two key limitations**:

1. **Does not support default arguments**: Parameters for Viper functions must be explicitly passed; default values are not supported.

```python
import micropython

# Correct example (no default arguments, 1 million operations, timed)
@micropython.viper
...
If you define a Viper function with default arguments and call it without passing arguments, an error occurs:
...
(The Viper compiler discards the default argument information, causing the function signature to become "requires 1 argument".)

2. **No optimization for floating-point operations**: Viper does not speed up floating-point operations.
...
# Viper function: same 100k floating-point operations (no optimization)
...
> Viper's floating-point time (2458ms) is very close to the normal function's time (2887ms), with only about a 15% speedup, far less than the 58x speedup for integer operations. This shows that **Viper is not suitable for floating-point intensive calculations**.

**Viper Pointer Types (ptr8/ptr16/ptr32) for Direct Contiguous Memory Access**:

Viper's pointer types are used for direct access to contiguous memory (e.g., `bytearray`), without bounds checking, and support single-element access via subscript (not slicing). A key optimization tip is: **perform the object-to-pointer conversion at the beginning of the function** (not inside the loop), because the conversion operation takes a few microseconds, which is amplified in large loops.
...
> - The Viper ptr8 version is **about 23 times faster**!
...
def viper_bad_convert(ba) -> int:
    total = 0
    for i in range(10000):
        buf = ptr8(ba)    # Convert every loop, cumulative time cost
        total += buf[i]
    return total
...
> - The difference is about **5 times**! In a loop of 10000 iterations, executing `ptr8(ba)` each time (each taking a few microseconds, accumulating significantly) is avoided by performing the conversion only once at the beginning.

Viper's integers are at the machine word level. On 32-bit hardware, arithmetic operations are performed modulo 2^32 (overflow truncation occurs). This is a trade-off of compatibility for performance, and this characteristic is more pronounced under heavy computation.

### 2.2.4 Computation and Hardware Optimization

#### 2.2.4.1 Replace Floating-Point Operations with Integer Operations

Chips without an FPU (Floating Point Unit) execute floating-point operations extremely slowly. Use integer operations in performance-critical sections, and only convert to floating-point in non-critical sections.
...
print("=== ADC Read Performance Comparison ===")
...
> - The floating-point version is about **57%** slower than the integer version. The difference will be further amplified as the number of ADC samples increases.
> - **Optimization Suggestion**: Perform only integer ADC readings inside the loop, and do a single floating-point conversion after the loop (if needed), or only convert when displaying output.

#### 2.2.4.2 Direct Register Read/Write

Bypass MicroPython's hardware abstraction layer and directly read/write chip registers, eliminating method call overhead (e.g., fast LED blinking, high-frequency GPIO operations).
...
# --------------------------
# SIO module base address (RP2040 fixed)
...
# Initialize: Set GPIO25 to output mode (execute only once, atomic operation) 
mem32[GPIO_OE_SET] = PIN25_MASK

# Initialize Pin object (execute only once)
...
REPL definition code (showing SIO register address configuration):
...
> - The register method is **about 48% faster** (1.5x)
> - In the SIO loop function, `GPIO_OUT_SET`, `GPIO_OUT_CLR`, and `PIN25_MASK` are cached to local variables, reducing global variable lookup overhead, making the SIO performance advantage more prominent.
> - **Further Optimization**: Combining the `@micropython.viper` decorator with `ptr32` pointers to directly write to registers can push GPIO toggle speeds into the MHz range.

#### 2.2.4.3 DMA Related Operations

In scenarios involving computation and hardware data interaction (e.g., batch ADC data acquisition, high-frequency GPIO signal output, sensor data stream reading), the CPU often spends a significant amount of time on **data transfer operations** (e.g., reading data from peripheral registers to memory, writing computation results to GPIO registers), consuming resources needed for computation. The RP2040's DMA (Direct Memory Access) controller can autonomously perform bulk data transfers between memory and peripherals/registers without CPU intervention. Its core optimization value is: **freeing the CPU from tedious data transfer tasks, allowing it to focus on core computation logic**.

From a development and performance perspective, MicroPython's support for RP2040 DMA is basic, only enabling simple bulk data transfers. In contrast, C (Pico SDK) can fully configure DMA transfer modes, trigger conditions, and data processing rules. Combined with direct register operations, it enables seamless optimization of computation and hardware data interaction, making it the optimal choice for high-throughput, low-latency scenarios.

# 3. Optimization Experiments

## 3.1 LCD Screen Optimization
...
| Optimization Method | Test Scenario | Time Before Optimization | Time After Optimization | Speedup Factor |
|---------------------|---------------|--------------------------|-------------------------|----------------|
| `@micropython.viper` (integer operations) | 1M `i*2+5` accumulations | 17221.732ms | 296.095ms | **~58x** |
...
| Cache object references | 100 attribute access writes | 0.999ms | 1.039ms (example) | ~Equal (more significant in large loops) |
| `const()` constant | 100k loop accessing constant | 1120.892ms | 1120.690ms | ~Equal in REPL (more significant with precompilation) |
...
- [MicroPython Official Performance Optimization Documentation](https://docs.micropython.org/en/latest/reference/speed_python.html)
