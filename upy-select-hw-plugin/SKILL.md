...
description: Plugin-based workflow version of select-hw. Consumes phase_complete.payload.manifest_content from upy-analyze-plugin, performs board/MCU confirmation, MicroPython firmware verification, pin assignment, and BOM generation, and outputs phase_complete(select-hw) for the MPY firmware flashing stage.
---

# Plugin Workflow Hardware Selection and Pin Assignment Skill

## Role Definition

`upy-select-hw-plugin` is the `select-hw` phase in the long-running workflow protocol. It consumes the phase artifact from `upy-analyze-plugin` and prepares hardware facts for the subsequent "MicroPython firmware flashing step for the corresponding MCU".
...
- Verifying the firmware download entry and flashing tool type
...
During direct testing, reading from the session directory `phase_complete.analyze.json` is allowed, but the `payload.manifest_content` must still be extracted. Do not infer project state from `manifest_draft.json`, logs, or old conversations.

The current real direct test artifact from `upy-analyze-plugin` uses session isolation:
...
2. Direct test fallback: read `manifest_validated.json`
...
| `artifact_root` | Current project/test output root, e.g., user-provided `G:\test\test`; `phase_complete.payload.artifacts.file_list.files[].path` is resolved relative to this by default | `sessions/<session_id>` and phase artifacts |
...
`artifact_root` is the "current run artifact root directory", not the skill/resource root. For example, when `artifact_root=G:\test\test`, the artifact path should be `sessions/<session_id>/select_hw_draft.json`; if the host sets `artifact_root` to the current `session_root`, then the artifact path should be `select_hw_draft.json`. Validation commands, phase logs, and file manifests must use the same root convention.

The user-provided project directory (e.g., `G:\test\test`) defaults to `artifact_root`, not `resource_root`. Do not copy skill scripts, the boards directory, or partial skills to `artifact_root` just because `upy-select-hw-plugin` or `upy-analyze-plugin` is missing under `artifact_root`.
...
Phase logs, command history, and artifact descriptions must also use relative paths. Do not write the local plugin installation directory (e.g., the skill/plugin path under the user's home directory) as a business fact source.
...
All time fields must come from a unified runtime time source. Do not hardcode placeholder times:

- `timestamp` — Injected by Claude Code / plugin runtime, or obtained via `upy-project-gen-toolchain-spec/scripts/workflow_time.py`.
- `pin_review.confirmed_at` — Must be the real UTC time when the user confirmation occurred. Do not use date zero or sample placeholder values.
...
`phase_complete.timestamp` must be obtained by re-calling `workflow_time.py --json` after script validation passes, ensuring it is >= all referenced artifact `updated_at` and `created_at` values. Do not reuse `pin_review.confirmed_at` or an earlier time as `phase_complete.timestamp`.
...
| `idempotency_key` | recommended | Remains unchanged on retry of the same action |
...
V0 does not require `device_command`. If the host does not support `approval_request` or `script_run`, select-hw must not claim success.
...
Step 1B Load full board definition
  ...
  If missing or lacks pin_layout:
...
Step 3 Pin assignment
  ...
  If candidate board lacks pin_layout:
...
Step 7 Obtain phase completion timestamp
...
- If the user requests a board change in `select-hw` that crosses MCU, chip family, firmware target, or significantly changes the main controller's capability boundary, the upstream requirements must not be silently overwritten and a `success` output must not be produced.
- Such changes must output `partial`, with `next_phase=null`, `checkpoint.resume_step=load_upstream_manifest` or `board_select`, `reason=board_change_requires_analyze`, and remind the user to start a new conversation or re-run analyze/select-hw.
...
`select-hw` can only consume the real functional devices already confirmed in the upstream `upstream_manifest.devices[]`. It must not silently add/replace sensors, displays, actuators, audio modules, communication modules, or other functional hardware in pin assignments, BOM, or the final `manifest_content.devices[]`. If the user requests adding or replacing functional devices during this phase, output `partial` and return to analyze/select-hw to re-confirm the device list.

Hard rules:

- `hardware_plan.pinout[].device` must match the upstream `devices[].name`. Only power/supply/system items like `power`, `GND`, `3V3`, `5V`, `board`, `mcu` are exceptions.
- Functional hardware in `hardware_plan.bom[]` must be mappable to the upstream `devices[]` via `name`, `model`, `device`, or `selected_model`. Supporting materials like Dupont wires, breadboards, resistors, capacitors, pin headers, USB cables, enclosures, battery holders, power modules, and adapter boards do not need to be in the upstream device list.
- BOM fields like `url`, `link`, `product_url`, `shop_url`, `datasheet_url`, `supplier`, `sku`, `search_query`, `purchase_links` can be retained, but they are not a strong contract passed to generate/deploy and must not be a prerequisite for downstream success.
- All physical BOM items should uniformly retain `product_url`, `shop_url`, `datasheet_url`, `supplier`, `sku`, `search_query`, `purchase_links` according to `references/bom_item_link_index.template.json`; write empty strings or empty arrays for unknowns. Missing links must not block `select-hw` success or progression to the next phase.
- `search_query` is used by the plugin side to generate purchase entry points by region; by default, it is generated from a combination of `model`, `selected_model`, `module`, `part`, `name`. For the China region, the YourCee product information entry `https://www.yourcee.com/cpzl` can be retained, and the user can be instructed to open the page and search using `search_query`.
- `purchase_links[].link_type="site_entry"` indicates a site entry point, not a verified link to a specific product page; when displayed by the plugin, the YourCee text should be "Search on YourCee", not "Buy this product".
- Do not fabricate real store URLs, suppliers, or SKUs to fill in missing links; the link index is only a procurement display template and does not change the hardware fact boundary.
- If `select-hw` supplements a specific model for an upstream generic name, e.g., `OLED display` -> `SSD1306 OLED`, it should be recorded as a model supplement or BOM/model information for the original device, not appended as a new `devices[]` item; if the model affects drivers/API, it must remain mappable back to the original upstream device.

## approval_request: board_unavailable

When the user-specified specific board or `pre_selected_board.id` is not in `upy-analyze-plugin/boards`, do not fail immediately. First, sort by the same series, same `chip_family`, same firmware port, and similar functional requirements, and recommend a known board with `pin_layout` as a replacement; at the same time, give the user the option to manually describe the wiring.

These mutually exclusive actions must be provided:

| action value | meaning | subsequent behavior |
| --- | --- | --- |
| `use_recommended_similar` | Use the system-recommended known board of the same series/similar functionality | Continue with firmware verification and pin assignment |
| `select_known_board` | User selects another known board from the board library | Re-enter `board_select` |
| `manual_wiring_description` | User manually describes "MCU pin -> device pin" | Produce partial/checkpoint, wait for user to provide structured wiring |
| `save_partial` | Pause | Produce partial/checkpoint |

The manual wiring description requires an array, with each record specifying `mcu_pin`, `device`, `device_pin`, `signal`, `voltage`, `notes`. Example: `GPIO21 -> AHT20 SDA`, `3V3 -> AHT20 VCC`, `GND -> AHT20 GND`.
...
- Whether the default bus pins are actually exposed and whether they conflict with other devices
...
- Whether the VCC/GND/configuration pins of the peripheral module are controlled by the MCU or hardwired to power/ground
- Differences between board variants, schematic version, silkscreen, and actual module consistency
...
| `revise_pin_plan` | User requests re-assignment of one or more pins | Return to `pin_assignment` |
| `manual_wiring_description` | User manually describes wiring | Produce partial/checkpoint, wait for structured wiring |
| `save_partial` | Pause | Produce partial/checkpoint |
...
| `confirmed_at` | required when confirmed=true | Real UTC time of this confirmation, ISO-8601 format; must not use sample placeholder time or date zero |
...
- During the candidate generation phase, enumerate the complete board library from `resource_root/upy-analyze-plugin/boards/*.json`, skipping `_template.json` and documentation files; do not load only the selected board.
- When `requirements.mcu_specified` exists, match candidates by `mcu`, `chip_family`, `firmware.board_name`.
- If `pre_selected_board` comes from the plugin UI, confirmation can be skipped, but validation is still required.
- `selected_board.id` must correspond to `upy-analyze-plugin/boards/<id>.json`. After confirming the board, the full board JSON must be loaded; pin assignment must not be done using only the MCU name or `selected_board` summary.
- The full board JSON is the source of truth for `firmware`, `pin_layout`, `restricted_gpio`, `onboard_peripherals`. `selected_board` is only a UI summary.
- When no MCU is specified, the candidate pool must prioritize limiting to the Pico/RP2 series and ESP32 series; unless the requirements clearly need another series, do not prioritize recommending STM32, Teensy, Pyboard, etc.
- Default sorting when no MCU is specified: Pico/Pico W, ESP32 DevKit, ESP32-S3, ESP32-C3; output Top 1 and Top 2 alternatives after scoring based on requirements.
- Add points for ESP32 series and Pico W when WiFi/BLE is needed; add points for ESP32-S3 when AI/voice/camera is needed; add points for ESP32-C3 for low power/battery power; add points for Pico/Pico W for pure GPIO or beginner use; add points for ESP8266/Pico for extreme low cost, but ESP8266 should not outweigh Pico/ESP32 unless budget is the sole primary constraint.
- When the user-specified board is not in the board library, prioritize recommending a known board of the same series or with similar functionality and `pin_layout`; simultaneously send `approval_request(board_unavailable)`, allowing the user to select a known board or manually describe wiring.
- When the user requests a board change in `select-hw` that crosses MCU/chip family/firmware target, do not continue with a successful artifact; output partial/checkpoint, requiring a new conversation or returning to the analyze phase to re-confirm requirements.
- When `pin_layout` is missing, default to switching to a known board with similar functionality and `pin_layout`.
- `cold-driver` does not affect MCU recommendation, pin assignment, or BOM; however, `devices[].driver.source="cold-driver"` must be normalized to `devices[].driver.status="cold_driver_required"` for use by the pre-generate gate and `upy-gen-driver-plugin`. `driver.source` is retained as provenance and is not a workflow gate.
- select-hw is not responsible for confirming the real-time latest version of the MicroPython firmware. Firmware versions in the board library are only cache information; the formal flashing stage should check `firmware.url` for the latest release. The select-hw output should focus on retaining `firmware_url`, `firmware_board_name`, `flash_tool`.
...
- GPIO avoids boot/strapping, flash/PSRAM, USB OTG, read-only pins; conditionally usable pins can enter the plan, but the user must be prompted to verify in warnings/notes and `pin_plan_review`.
- Power and GND must be included in `pinout`.
- If the board JSON has `pin_options`, remapping is only allowed within the scope of `pin_options`; if it is a flexible matrix, hard-disabled pins must also be avoided. Conditionally usable pins are not schema hard failures; they are handled via warnings and user confirmation.
- Deviation from `pin_layout.default_bus_pins` must be explained in `pinout[].notes` and warnings.
- When the user provides wiring, prioritize retaining it, but it must pass the restricted/occupied validation of the board JSON; illegal user wiring must not silently succeed.
...
- When an onboard device matches a user-specified or system-recommended device, reuse the onboard default/occupied pins declared in `onboard_peripherals`, do not allocate additional external GPIOs, and do not add them to the BOM again.
- When an onboard device enters the project device list, do not change `devices[].source` to a new enum; `source` remains `user_specified` or `system_recommended`, and additionally write `physical_source="board_onboard"`, `onboard_peripheral_ref={board_id,index,name,type,model,occupied_pins,verification}`. This allows downstream legacy plugins to still read `devices[]` while recognizing it is not an external BOM item.
...
- GPIO summaries in `pin_assignment_log.md` and phase logs must be calculated from the full board JSON and the final `pinout`; static lists are not allowed. V0 must include at least `used_gpio`, `unused_gpio`, `restricted_or_occupied_gpio` groups; do not package conditionally usable pins as absolutely safe, just explain the limitations and confirmation points in warnings.
- If WiFi is enabled and GPIOs from `adc2_wifi_conflict` are used, all relevant GPIOs must be listed completely. Conflict only occurs when `pinout[].type=adc`; using them as digital signals like I2C/I2S/GPIO is allowed, but warnings or notes must state "WiFi only affects ADC readings, not digital usage".
- When using board default UART/REPL/USB serial port related pins as regular GPIO, it must be confirmed that the serial port is not used for debugging/communication, or the occupancy must be written into a warning with reallocation suggestions.
...
| `adc2_wifi_conflict` | Only conflicts when `type=adc` and WiFi is enabled; digital input/output is usable but should be noted | error for ADC; warning for digital use |
...
| `physical_pin` | optional | Board silkscreen/physical pin number, filled when data is available in the board library |
...
Structured `pin_decisions[]` must be generated and retained in the final `manifest_content`. Default buses, auto-assignments, user wiring, onboard peripheral reuse, and fixed power/ground must all have corresponding decisions; natural language notes can only supplement, not replace structured evidence.
...
| `requires_user_review` | yes | Whether the user is advised to focus on confirming this in `pin_plan_review`; V0 does not require precise coverage of every risk pin |
| `review_prompt` | optional | Prompt for the user to check the schematic/silkscreen/module documentation |
| `deviation` | optional | Structured explanation of deviation from default or occupied pin release |
...
If `reason_code=onboard_occupied`, `evidence_path` must point to `onboard_peripherals[].occupied_pins`, and `evidence_value` must match `from_gpio`; otherwise, it should be treated as `pin_decision_invalid` or enter `manual_review_required`, and the LLM must not assert via notes that a GPIO is occupied by an onboard device.
...
| `hardware_plan.mcu` | yes | MCU, firmware entry, and flashing tool |
| `hardware_plan.pinout` | yes | Pin assignment array |
| `hardware_plan.pin_decisions` | yes | Structured decisions and evidence for each pin selection; the script must validate and retain them in the final `manifest_content` |
...
| `metadata.idempotency_key` | recommended | Idempotency key for the manifest validation action |
...
BOM prices in V0 temporarily accept LLM common-sense estimates and do not connect to marketplace data sources.
...
Avoid using ambiguous names like "unused (idle)". `select_hw_phase_log.md` must record the complete step timeline, `runtime_context` parameters, and path conventions.
...
- Flashing firmware
- Connecting to the internet to check marketplace prices
...
- Pinout conflicts
- `pin_decisions` fields, enums, evidence, and deviation are valid and correspond to the final `pinout`
...
- File artifact declared relative paths actually exist
...
- Three sources (user wiring, onboard peripheral reuse, external device auto-assignment) are distinguishable
...
The second command is used to validate that the script artifact still conforms to the normalized `manifest_content` schema; the formal phase completion still requires `--validate-phase-complete` to validate `phase_complete.select_hw.json`.
...
3. Trigger `approval_request(board_select)` when `mcu_specified` exists but no `pre_selected_board`.
...
11. Validator covers board-root, restricted pins, default bus deviations, user wiring, onboard peripheral reuse, ADC2/WiFi digital usage warnings.
12. `phase_complete.payload.artifacts` covers all formal artifacts, and logs and artifacts do not contain the local plugin installation absolute path.
...
14. When the user specifies an illegal GPIO, it must not silently auto-change pins; must output partial/checkpoint or validation failure.

## Maintenance Principles

Subsequent updates should be based on the contents of the `upy-select-hw-plugin` directory, and then update the course documentation in reverse.
