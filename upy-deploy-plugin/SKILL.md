...
After deployment completes, it does not silently end. It must display a deployment results tab and read user feedback:

| User Choice | Action |
| --- | --- |
| Regenerate | `upy-generate-plugin(mode=fix, source=user_feedback_after_deploy)` |
| Auto-debug | `upy-autofix-plugin` |
| End and upload project library | Enter project library upload/publish flow |

On FAIL, priority is given to entering `upy-autofix-plugin`. If `upy-autofix-plugin` is not yet implemented, it can fall back to `upy-generate-plugin(mode=fix, source=deploy_fail)`.
...
`deploy_strategy` supports:

| Value | Meaning |
| --- | --- |
| `upload_only` | Do not clean device files; directly upload the current project |
| `clean_then_upload` | Perform a standard clean of old project files and business directories, then upload |
| `erase_then_upload` | Clean all listable files/directories on the device before uploading; requires dry-run and double confirmation |

`erase_then_upload` is not equivalent to re-flashing the MicroPython interpreter firmware. It only cleans files/directories in the MicroPython filesystem.
...
5. Send `approval_request(deploy_port_select)` for the user to select the real port.
6. Send `approval_request(deploy_strategy_select)` for the user to select the deployment strategy.
7. If cleaning is selected:
   ...
   - `project_files` cleaning must cover old production-forbidden artifacts, including `conf.mpy`, `boot.mpy`, `main.mpy`, `board.mpy`, and `drivers/**/mock.py|mock.mpy`. Otherwise, even if the new upload is correctly filtered, the device may still run old files.
...
11. Capture persistent REPL output using the standalone `scripts/capture_repl.py`. It is recommended to call `scripts/capture_repl.py --reset-first --duration-ms <ms>` after upload, so the script first enters REPL, sends Ctrl-D for a soft reset, and continuously reads startup output; do not reset/wait before starting to listen, otherwise startup tracebacks from `main.py` may be missed.
12. Read device-side logs:
    - Before deployment, a log strategy option should be provided: keep old logs, read and download old logs, or clear old logs before deployment.
    - `project/tools/read_device_log.py`
    - `project/tools/log_report.py`
    - Logs can only be cleared after user confirmation, by calling the project tool's `--clear` or the clean script's `--include-logs`; do not silently delete old logs by default.
...
16. Before outputting `phase_complete`, `scripts/deploy_manifest.py --input <phase_complete> --validate-phase-complete` must be run; if it fails, the deploy must not be judged as success.
...
After upload, soft reset, waiting for device recovery, and reading device logs, it is recommended to ask whether to run device-side contract tests. The default suggestion is to run, but skipping must be allowed, as some projects' device tests may touch real hardware, or the user may only want a quick upload for observation.
...
Optional user feedback text must be collected, such as actual device behavior, mpremote output, serial errors, manually observed issues, and device log summaries. When the user chooses to regenerate, `error_context` must be passed.
...
Empty REPL output should not directly result in FAIL. If serial capture is empty, but upload/clean succeeded, log report `error_count=0`, and device tests did not fail, output `PASS_WITH_WARNINGS` and add a warning, e.g., `serial capture produced no output`. This is because the application may only write to a rotating file logger or have no stdout at runtime.

## Device Tools Area

In addition to the main workflow, the UI can provide a standalone "Device Tools" area:
...
| Script | Purpose |
| --- | --- |
| `scripts/check_environment.py` | Check for `mpremote`, optional `pyserial`, and installation instructions |
...
- `mpremote resume exec` should only be used for short probes or one-time actions like pre-deployment cleaning; do not use repeated `resume exec` as a substitute for persistent REPL listening.
...
`manifest_content` must retain the complete upstream manifest and then append deploy facts; it must not only write a summary.

`phase_complete.payload.deploy_result` must come from the structured result of `scripts/deploy_result.py` or be consistent with it field by field. The LLM can summarize the result, but must not manually rewrite blocking failures from underlying `mip_install_result.json`, upload summary, device tests, log report, or REPL capture into PASS.
...
- `erase_then_upload` must have a dry-run and double confirmation.
