...
- Target devices, OS installation, desktop emulation, Web runtime: `mpos-dev/reference/docs-deploy-targets.md`
- Browser emulation details: `mpos-dev/reference/docs-web-port.md`
- MPK, AppStore, upystore: `mpos-dev/reference/docs-packaging.md`

Prefer JSON for API decisions. LVGL `type_aliases[]` only explains signature types; they are not runtime APIs that can be generated.
...
1. Read user requirements and existing context. Preserve the app name, features, target device, hardware, and publishing intent explicitly specified by the user.
...
4. Determine app structure: which Activities are needed, whether a Service is required, whether `boot_completed`, Intents, persistence, or background tasks are needed.
...
- `app_structure.manifest` defaults to root `MANIFEST.JSON` for new apps.
- `app_structure.icon` defaults to root `icon_64x64.png` for new apps.
...
- External Python driver, component datasheet, dependency organization needed: `mpos-prepare-deps`.
- User only wants to verify an existing app: `mpos-test-app`.
- User mentions running, emulation, real device installation, flashing: `mpos-deploy-app`, and clarify that app installation is not firmware flashing.
