...
After completing packaging and producing `package_result.json`, you must register in the project state directory:
...
- Local strong constraints: `<repo-root>/AGENTS.md`
...
- Do not run the desktop simulator, Web Port, or device interaction; running tests returns to `mpos-test-app`.
- Do not install to the real `/apps`, do not overwrite the real `internal_filesystem/apps`.
- Do not log in, upload, or save upystore credentials; publishing/uploading is the user's responsibility or for a future `mpos-publish-app`.
...
Compatible with the old layout, but must warn:
...
If both the root `MANIFEST.JSON` and the old `META-INF/MANIFEST.JSON` exist, the root manifest takes precedence, and a warning about the coexistence of the old path is recorded. The same applies to the icon; the root `icon_64x64.png` takes precedence.
...
2. Read optional `generation_result.json` and `app_test_result.json`. Missing or failed results only produce warnings and do not block packaging.
   - Missing, path does not exist, JSON cannot be parsed, schema/phase mismatch, or `result != "success"` are all treated as warnings.
...
5. Add `--install-check` when the user requests temporary install validation. This check only extracts to `tmp/mpos-package-app/<fullname>/install-check/` and re-validates the unpacked App, without writing to the real App directory:
...
Must be guaranteed by the script:
...
- The default compression method is `stored`. `--compression deflated` can be explicitly used, but local-header validation must still pass.
...
- Use a fixed timestamp and stable sorting for reproducible builds.

## App Index

This skill only generates the `app_index_entry.json` for the current App by default. It does not merge or modify the complete `app_index.json`. Full store index merging involves release repositories, URL strategies, sorting, conflict resolution, and go-live processes, which are not the default responsibility of this skill.

`emit_app_index_entry.py` will generate single metadata entries based on the manifest and generate them according to the base URL:
...
- `checks[]` must at least include `app_validation`, `generation_result`, `app_test_result`, `mpk_validation`, `app_index_entry`.
- When `--install-check` is requested, append `temporary_install_validation`.
- Missing/failed generation/test results only record warnings and must not cause the package to fail.
