...
- `pin_layout.pin_options`: Only fill this for chips like RP2040/Pico where pins and peripherals are fixed. For ESP32 series, leave as empty object `{}`
...
| `mcu` | string | Full MCU model, appears in the BOM. E.g., `ESP32-S3-WROOM-1` |
...
| `firmware.latest_version` | string | Known latest firmware version |
...
**★★★ Affects AI selection and code generation, must be accurate:**
...
| `spi` | number | Many SPI devices but few controllers → suggests shared bus |
| `wifi` | boolean | Matches networking requirements with board capabilities |
| `ble` | boolean | Matches Bluetooth requirements with board capabilities |

**★★ Referenced during pin allocation, try to be accurate:**

| Field | Type | How AI Uses It |
|------|------|---------|
| `pwm` | number | Servo/LED dimming requires PWM channels |

**★ Displayed on the board details page; filling is better, but not filling does not affect AI decisions:**
...
| `notes` | string | Notes for the user. **The LLM also reads this section for decision-making.** Write key technical limitations, compatibility warnings, and critical differences from other boards |

### 3.5 Onboard Peripherals `onboard_peripherals` (Optional)

**When to fill:** The board comes with built-in peripherals like screens, sensors, buttons (not just indicator LEDs).

**When to use an empty array:** Pure GPIO development boards, e.g., ESP32 DevKit only has an onboard LED; write one entry or use an empty array.
...
| `occupied_pins` | object | Yes | Which pins are occupied. Key = function name, value = GPIO number. -1 means no GPIO used |
...
| `always_used` | boolean | Yes | true = this pin is definitely occupied, skip during pin allocation. false = user can disable this peripheral, freeing the pin |
...
| `install_cmd` | mpremote install command |

**Note:** Do not fill in uncertain drivers. Only fill driver information after verification that the driver works on this board with this peripheral. Otherwise leave it empty and let the AI search.
...
**Essentially "tell the AI to avoid the minefield, the rest is free to allocate."**
...
|------|------|------|
| `input_only` | Can only be input, cannot output/pull up/down | ESP32 GPIO 34-39 |
| `strapping` | Determines boot mode at startup; incorrect connection may prevent boot | ESP32 GPIO 0/2/5/12/15 |
| `flash_psram_occupied` | Internally occupied by Flash/PSRAM, not externally visible | ESP32 GPIO 6-11 |
| `adc2_wifi_conflict` | ADC2 unreliable when WiFi is on | Most ESP32 ADC2 channels |
| `usb_otg_pins` | Dedicated to USB OTG; repurposing loses USB functionality | ESP32-S3 GPIO 19/20 |
| `usb_serial_pins` | USB serial pins for debugging and flashing | ESP32-C3 GPIO 18/19 |
| `boot_fail_risk` | May not always cause boot failure, but historically problematic | ESP8266 GPIO 0/1/2/3/9/10 |
...
**Essentially "tell the AI each function can only be selected from a specific set of pins."**

`default_bus_pins`: Same as above, fill as an array.

`restricted_gpio`: Same as above, fill pins that cannot be occupied.

`pin_options`: **This is the key field for the fixed model.** List the selectable pins for each peripheral function:
...
| `action` | `boost` (add points) or `exclude` (remove) |
...
2. For each board: boost rules matching the chip_family → add points; exclude rules matching → remove
3. The two boards with the highest scores are recommended
4. Exclusion does not necessarily mean the board is bad; it may be unsuitable for the current scenario (e.g., excluding a board with only 1 I2C bus when multiple I2C devices are needed)
...
**Local testing phase:** The plugin reads JSON files directly from this directory.

**Production phase:** The server provides an API, the plugin calls:
```
GET /v1/boards
→ { version: "2.0", boards: [...] }
```

The server scans all `*.json` files in this directory (excluding `_template.json` and `matching-rules.json`) at startup, merges them, and returns the result.
...
4. Fill in specs (check the chip datasheet, fill all 13 items according to actual conditions)
...
6. Fill in onboard_peripherals:
   ...
   - Board has a screen/sensor/buttons → for each peripheral, fill occupied_pins and whether there is a driver link
7. Fill in pin_layout:
   - ESP32/ESP8266 → model: "flexible", fill default_bus_pins + restricted_gpio, leave pin_options empty
   - RP2040/Pico → model: "fixed", fill all fields including pin_options
