...
Transform the `dependency_plan.items` from `mpos-analyze-app` into a dependency handoff that can be passed to `mpos-gen-app`. Only handle dependencies available at the application layer: pure Python files, MicroPython packages, source code that can be vendored with the App, and MPY runtime dependencies that are already available.
...
Cache remains in `tmp/mpos-deps-cache/<fullname>/`; the project state only records the handoff, cache path, and summary, without copying cache contents.
...
If input comes from `mpos-analyze-app`, read its JSON. If no JSON is available, first organize equivalent `app.fullname`, target functionality, hardware/protocol, and dependencies from user requirements.

The upypi / awesome-micropython search flow from `upy-pkg-guide` can be reused, but async search strategies must be added on top; do not search only by synchronous drivers.
...
- Do not generate business App code; only write adapter recommendations for non-blocking wrappers of synchronous libraries, leaving implementation to `mpos-gen-app`.
...
| Third-party source requiring isolation | `assets/vendor/<dependency>/...` | Only used when marked `requires_vendor_path_injection=true` in handoff |
...
If the App directory does not yet exist, do not generate manifests or business code for dependencies. Runtime files can be downloaded to:
...
`--mode auto` rules: if `internal_filesystem/apps/<fullname>/` already exists, write to the App's `assets/`; if the App has not been created, write to `tmp/mpos-deps-cache/<fullname>/staged/assets/`. Do not download READMEs, examples, or package metadata as runtime files to the App.
...
For each dependency, use at least two sets of searches: base terms and async terms:
...
Cache must not contain accounts, tokens, private network credentials, or user device information. Cache files must not be packaged as App runtime files unless the user explicitly requests license or attribution to be included in the App.
...
And `adapter_requirements[]` must describe how `mpos-gen-app` needs to wrap it, for example:
...
5. Check candidate dependencies for application-layer compatibility, MicroPython compatibility, async support, blocking patterns, license, file size, and import paths.
...
1. `Dependency Locations`: Explain that runtime files are ultimately placed under `internal_filesystem/apps/<fullname>/assets/`, and cache is placed under `tmp/mpos-deps-cache/<fullname>/`.
...
- `schema_version` is fixed as `"mpos-prepare-deps-v1"`.
- `phase` is fixed as `"prepare-deps"`.
